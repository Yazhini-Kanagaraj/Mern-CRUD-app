...
const API = "https://mern-crud-app-5q7z.onrender.com";
function App() {
  //State to store the value from database
  const[people,setPeople]=useState([]);
  const [form,setForm]=useState({name: "",age: ""});
  const[editId,setEditId]=useState(null);  //Read from database
  useEffect( ()=>{
    loadpeople();
  },[]);
  ...
  const addPerson = async ()=>{
    ...
    const res=await axios.post(API + "/add", {name:form.name,age: Number(form.age)});
